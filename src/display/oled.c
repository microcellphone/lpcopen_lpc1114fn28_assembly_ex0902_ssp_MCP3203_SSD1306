...
void OLED_Config_Request(void)
{
	...
	//	SSD1306_CS_SET()
	Chip_IOCON_PinMuxSet(LPC_IOCON, IOCON_PIO0_0, (IOCON_FUNC1 | IOCON_MODE_INACT));
	Chip_GPIO_SetPinDIROutput(LPC_GPIO, 0, 0);   // PIO0_0 out
	Chip_GPIO_SetPinState(LPC_GPIO, 0, 0, true); // high
...
{
	OLED_DrawChar((char)data, Font_11x18, White);
}
